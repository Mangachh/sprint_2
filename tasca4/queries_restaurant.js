...
db.restaurants.find({borough: "Bronx"}).skip(5).limit(5)
 ...
 // Escriu una consulta per trobar els restaurants quins no preparen cap cuisine de 'American' i 
...
db.restaurants.find({'grades.score': {$lt: 10}},{restaurant_id: 1, name: 1, borough: 1, cuisine: 1 })
 ...
 // Escriu una consulta per trobar el restaurant_id, name, borough i cuisine per a aquells restaurants que preparen peix excepte 'American' i 'Chinees' 
 // o el name del restaurant comença amb lletres 'Wil'
 ???????????????? -> no sé quien prepara pescado
 
 // Escriu una consulta per trobar el restaurant_id, name, i grades per a aquells restaurants que aconsegueixin un grau "A" i un score 11 en dades d
 ...
 // Escriu una consulta per trobar el restaurant_id, name i grades per a aquells restaurants on el 2n element de varietat de graus conté 
 ...
 // Escriu una consulta per trobar el restaurant_id, name, adreça i ubicació geogràfica per a aquells restaurants on el segon 
 ...
 ?????????????????????? -> no sé que se supone que debo buscar
